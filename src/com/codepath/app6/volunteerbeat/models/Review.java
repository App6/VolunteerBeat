...
public class Review implements Parcelable {
	...
	public Review(JSONObject json) {
		super();
		try {
			this.userName = json.getJSONObject("reviewer").getString(
					"screenName");
			this.rating = (float) json.getDouble("rating");
			this.reviewDate = "---";

			this.reviewText = json.getString("review");
	...
	public static ArrayList<Review> fromJSONArray(JSONArray jarray) {
		ArrayList<Review> aReviews = new ArrayList<Review>();
		
		for (int i =0; i < jarray.length(); i++) {
			Review r = null;
			try {
				r = new Review(jarray.getJSONObject(i));
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
